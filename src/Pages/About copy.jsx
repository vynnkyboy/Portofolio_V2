...
const AboutPage = () => {
  ...
  // Update count jika localStorage berubah (misal setelah fetch di Portofolio)
  useEffect(() => {
    const updateCounts = () => {
      const storedProjects = localStorage.getItem("projects");
      const storedCertificates = localStorage.getItem("certificates");
      setProjectCount(storedProjects ? JSON.parse(storedProjects).length : 0);
      setCertificateCount(storedCertificates ? JSON.parse(storedCertificates).length : 0);
    };
    window.addEventListener("storage", updateCounts);
    // Jaga-jaga update saat mount
    updateCounts();
